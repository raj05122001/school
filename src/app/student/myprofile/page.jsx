...
import { changeProfilePassword, getStudentDetails, getTeacherDetails, updateStudentDetails, updateTeacherDetails } from "@/api/apiHelper";
...
console.log("Password Data", passwordData)
  ...
  const fetchStudentDetails = async () => {
    try {
      ...
      setValue("full_name", fetchedData?.user?.full_name || "")
      setValue("roll_no", fetchedData?.roll_no || "")
      setValue("batch_year", fetchedData?.batch_year || "")
      setValue("email", fetchedData?.user?.email || "")
      setProfilePicUrl(`${BASE_URL_MEET}${fetchedData?.user?.profile_pic}`);
      ...
      // });
      setInitialData({
        full_name: fetchedData?.user?.full_name || "",
        roll_no: fetchedData?.roll_no || "",
        batch_year: fetchedData?.batch_year || "",
        email: fetchedData?.user?.email || "",  
        profile_pic: `${BASE_URL_MEET}${fetchedData?.user?.profile_pic}`,
      });
    } catch (error) {
      console.error(error);
    }
