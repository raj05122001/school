      ...
      if (error.code === "ERR_NETWORK") {
        ...
        return {
          audioProcess: {
            ...
            errorMessage:
              "Could not save and upload your Lecture due to internet connectivity issue. PLEASE DO NOT CLOSE YOUR BROWSER. We will retry uploading the Lecture when connection is restored.",
          },
        };
      } else {
...
export const replaceSpeaker = (text) => {
  if (
    !text?.match(/^Unidentified ([A-Za-z0-9_]+)(?:,([^@]+@[^,\s]+))?( .+)?$/)
  ) {
    return text?.split(",")[0];
  }
  return text
    ?.split(",")[0]
    ?.replace(/SPEAKER_(\d+)/g, function (match, digits) {
      const numericValue = parseInt(digits, 10);

      if (numericValue >= 0 && numericValue < 24) {
        const replacement = String.fromCharCode(
          "A".charCodeAt(0) + numericValue
        );
        return replacement;
      }
